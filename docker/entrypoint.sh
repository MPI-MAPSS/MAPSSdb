...
start_celery_supervisor() {
	cd ${APP_FOLDER}
	supervisord -c docker/mapss-supervisor.conf
}
...
run_django_server() {
	...
	cd ${APP_FOLDER}
    echo "Running Django"
	exec sh -c "pip install debugpy -t /tmp && python3 -Wdefault /tmp/debugpy --listen 0.0.0.0:5679 manage.py runserver 0.0.0.0:${DJANGO_PORT}"
}
